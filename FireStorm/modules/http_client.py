    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=data) as response:
            if response.status == 200:
                ...
                # пытаемся скачать подпись (если существует на сервере)
                sig_url = f'{URL}/getupdate_v2_sig'
                try:
                    async with session.post(sig_url, data=data, timeout=30) as sig_response:
                        if sig_response.status == 200:
                            sig_filename = f'{filename}.asc'
                            with open(sig_filename, 'wb') as sig_file:
                                sig_file.write(await sig_response.read())
                            print(f"Signature '{sig_filename}' received and saved.")
                except Exception:
                    pass
                bar.complete()
                return True
